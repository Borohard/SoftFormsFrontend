            ...
            <Grid container spacing={1}>
                ...
                {item.options &&
                    ...
                    item.options.map((option, key) => (
                        <Box style={{display:'flex'}}>
                          <TextField className="px-2 pt-2 h-10 rounded-md w-full focus:outline-fuchsia-800"
                              fullWidth
                              placeholder={`Вариант ${key + 1}`}
                              defaultValue={option?.value}
                              key={option?.id}
                              onChange={(e) =>
                                  handleOptionValues(item?.id, option?.id, e.target.value)
                              }
                          />

                          <Tooltip className="pb-3" title="Удалить вариант" aria-label="delete-option">
                            <IconButton
                    ...
                    <button className="mt-5 ml-1.5 inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-fuchsia-700 rounded-lg hover:bg-fuchsia-800 "
              ...
              <Grid item xs={3}>
                <FormControl fullWidth className="text-center">
                  <InputLabel id="el-type-label">Тип вопроса</InputLabel>
                  <Select
